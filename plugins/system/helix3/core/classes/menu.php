...
class Helix3Menu {
	...
	public function initMenu(){
		...
		foreach ($items as &$item) {
			...
			switch ($item->type) {
				// Flex Note: removed "continue" due to error in PHP. Changed to "break".
				case 'separator':
                break;
	...
	private function item($item, $extra_class=''){
		...
		//Add Menu FontAwesome Icon (changed to FontAwesome 5 since Flex 3.9)
		if($icon = $item->params->get('icon')) {
			
			if(strpos($icon, "fab") !== false || strpos($icon, "fas") !== false || strpos($icon, "far") !== false) {
				// FontAwesome 5
				$fa_icon = str_replace("fa ", "", $icon);
			} else {
				// FontAwesome 4
				$fa_icon = 'fa ' . $icon;
			}
			...
			if($showmenutitle) {
				$linktitle = '<i class="' . $fa_icon . '"></i> ' . $linktitle;
			} else {
				$linktitle = '<i class="' . $fa_icon . '"></i>';
			}
		}
